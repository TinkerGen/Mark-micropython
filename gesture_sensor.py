...
PAJ7620_BANK0		= PAJ7620_VAL(0,0)
PAJ7620_BANK1	= PAJ7620_VAL(1,0)

#  PAJ7620_ADDR_SUSPEND_CMD
PAJ7620_I2C_WAKEUP	= PAJ7620_VAL(1,0)
PAJ7620_I2C_SUSPEND	= PAJ7620_VAL(0,0)

#  PAJ7620_ADDR_OPERATION_ENABLE
PAJ7620_ENABLE		= PAJ7620_VAL(1,0)
PAJ7620_DISABLE		= PAJ7620_VAL(0,0)
"""
typedef enum [
	BANK0 = 0,
	BANK1,		
...
"""
GES_RIGHT_FLAG			=	PAJ7620_VAL(1,0)
GES_LEFT_FLAG			=	PAJ7620_VAL(1,1)
GES_UP_FLAG				=	PAJ7620_VAL(1,2)
GES_DOWN_FLAG			=	PAJ7620_VAL(1,3)
GES_FORWARD_FLAG		=	PAJ7620_VAL(1,4)
GES_BACKWARD_FLAG		=	PAJ7620_VAL(1,5)
GES_CLOCKWISE_FLAG		=	PAJ7620_VAL(1,6)
GES_COUNT_CLOCKWISE_FLAG	= PAJ7620_VAL(1,7)
GES_WAVE_FLAG				= PAJ7620_VAL(1,0)
...
class GestureSensor(object):
    def __init__(self, addr=PAJ7620_ID):
        self.i2c_device = I2C(I2C.I2C0, freq=100000, scl=30, sda=31)
        time.sleep(0.01)

    def write(self, buf=None):
        if buf is not None:
            self.i2c_device.writeto(PAJ7620_ID, buf)

    def paj7620WriteReg(self, addr, cmd):
        if cmd is not None:
            self.i2c_device.writeto_mem(PAJ7620_ID, addr, cmd & 0xFF, mem_size=8)

    def paj7620ReadReg(self, addr,  qty):
        dta_send = bytearray([addr])
        self.write(dta_send)
        time.sleep(.001)
        num = qty
        bytes=self.i2c_device.readfrom(PAJ7620_ID, num)
        for _b in bytes:
            value = int(_b)
        return value
    ...
    def paj7620SelectBank(self, bank):
        ...
        if (bank == BANK1):
            self.paj7620WriteReg(PAJ7620_REGITER_BANK_SEL, PAJ7620_BANK1)

    def paj7620Init(self): 
        ...
        data0 = self.paj7620ReadReg(0, 1) 
        data1 = self.paj7620ReadReg(1, 1) 

        if ( (data0 is not 0x20 ) or (data1 is not 0x76) ):
            return 0xff 
        if ( data0 == 0x20 ):
            pass
        ...
        #print("Set up gaming mode.") 
        self.paj7620SelectBank(BANK1)   # gesture flage reg in Bank1
        #  paj7620WriteReg(0x65, 0xB7)  #  far mode 120 fps
        self.paj7620WriteReg(0x65, 0x12)   #  near mode 240 fps

        self.paj7620SelectBank(BANK0)   # gesture flage reg in Bank0
        
        #print("Paj7620 initialize register finished.") 
        return 0
        
    def getGestureValue(self, addr, opt):
        GestureValue = {
        'GES_RIGHT_FLAG':GES_RIGHT_FLAG,
        'GES_LEFT_FLAG':GES_LEFT_FLAG,
        'GES_UP_FLAG':GES_UP_FLAG,
        'GES_DOWN_FLAG':GES_DOWN_FLAG,
        'GES_FORWARD_FLAG':GES_FORWARD_FLAG,
        'GES_BACKWARD_FLAG':GES_BACKWARD_FLAG,
        'GES_CLOCKWISE_FLAG':GES_CLOCKWISE_FLAG,
        'GES_COUNT_CLOCKWISE_FLAG':GES_COUNT_CLOCKWISE_FLAG,
        'GES_WAVE_FLAG':GES_WAVE_FLAG}
        while 1:
            data = self.paj7620ReadReg(addr, 1)
            #print(data)
            if(data is not 0):
                return data == GestureValue[opt]
...
while True:
    time.sleep_ms(500)
    Gesture.getGestureValue(0x43,'GES_RIGHT_FLAG')
"""